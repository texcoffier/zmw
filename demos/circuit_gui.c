...
{
  *x = g->x + g->w + zMw[-1].u.size.allocated.x ;
      ...
      for(i=0; c->gates[i].type != Gate_EOT; i++)
	{
	  ...
	  ZMW(zmw_anchor_move_with_init_values(&c->gates[i].x, &c->gates[i].y
	      ...
	      ZMW(zmw_box())
		{
		  ...
		  zmw_border_width(0) ;
		  zmw_image_from_file_activable(icones[c->gates[i].type], &pb[c->gates[i].type]) ;
		  ...
		  switch( number_of_inputs[c->gates[i].type] )
		    {
		    case 2:
		      zmw_x(-ZMW_BORDER_WIDTH) ;
		      zmw_y(0) ;
		      gate_input(&input, 2*i) ;
		      zmw_x(-ZMW_BORDER_WIDTH) ;
		      zmw_y(zMw[-1].u.size.allocated.height - gate_height) ;
		      gate_input(&input, 2*i+1) ;
		      break ;
		    case 1:
		      zmw_x(-ZMW_BORDER_WIDTH) ;
		      zmw_y((zMw[-1].u.size.allocated.height - gate_height)/2) ;
		      gate_input(&input, 2*i) ;
		      break ;
		    }
		  zmw_x(zMw[-1].u.size.allocated.width - gate_width + ZMW_BORDER_WIDTH) ;
		  zmw_y((zMw[-1].u.size.allocated.height - gate_height)/2) ;
		  ...
		  zmw_x(zMw[-1].u.size.allocated.width) ;
		  zmw_y(0) ;
...
{
  static GdkWindow *history_window = NULL ;
  int i ;
  char buf[99] ;

  zmw_auto_resize(1) ;
  ZMW(zmw_window(&history_window, "Command History"))
    {
...
{
  static int file_detached=0, edit_detached=0 ;
  static GdkWindow *file_window = NULL, *edit_window=NULL ;
  static int display_history = 0 ;

  static GdkWindow *filechooseropen = NULL, *filechoosersave = NULL ;
  static Zmw_Boolean filename_to_open = 0, filename_to_save = 0 ;

  static Zmw_Boolean alert = 0 ;
  static GdkWindow *alert_window = NULL ;
  ...
  ZMW(zmw_box_horizontal())
    {
      ...
      zmw_button("File") ;
      if ( zmw_window_is_popped(file_detached) )
	ZMW(zmw_window_popup(&file_window, file_detached))
	{
	  ZMW(zmw_box_vertical())
	    {
	      zmw_tearoff(&file_detached, &file_window) ;
      ...
      zmw_button("Edit") ;
      if ( zmw_window_is_popped(edit_detached) )
	ZMW(zmw_window_popup(&edit_window, edit_detached))
	{
	  ZMW(zmw_box_vertical())
	    {
	      zmw_tearoff(&edit_detached, &edit_window) ;
  ...
  zmw_filechooser(&filename_to_save, &filechoosersave
  ...
  zmw_filechooser(&filename_to_open, &filechooseropen
  ...
  zmw_alert(&alert_window, alert_message, &alert) ;
...
void circuit_mode(const Circuit *c, Type *s)
{
  ...
  zmw_foreground(ZMW_BACKGROUND_POPED, 0x00D0D0D0) ;
  zmw_horizontal_expand(0) ;
  ZMW(zmw_box_vertical())
    {
      for(i=0; names[i]; i++)
	{
	  if ( i == *s )
	    zmw_foreground(ZMW_BACKGROUND_POPED, 0x0000FF00) ;
	  if ( icones[i][0] )
	    zmw_image_from_file_activable(icones[i], &pb[i]) ;
	  else
	    zmw_button(names[i]) ;
	  if ( i == *s )
	    zmw_foreground(ZMW_BACKGROUND_POPED, 0x00D0D0D0) ;
...
void circuit()
{
  ...
  static Zmw_Boolean sb_needed=0 ;
  int tx, ty ;
  char tmp[999] ;

  tx = ty = 0 ;  /* To avoid a compiler warning */
	  ...
	  ZMW(zmw_box_horizontal())
	    {
	      ...
	      ZMW(zmw_viewport_with_scrollbar_if_needed(&x, &y,&sb_needed))
		{
		  zmw_width(c.w) ;
		  zmw_height(c.h) ;
		  gate_area(&c, s) ;
		  tx = zmw.x - ZMW_SIZE_ALLOCATED.x ;
		  ty = zmw.y - ZMW_SIZE_ALLOCATED.y ;
		}
	      if ( s < Change_Input && zmw_button_pressed() )
		circuit_gate_add(&c, s, tx, ty) ;
	    }
	}
    }
}
