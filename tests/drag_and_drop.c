	  ...
	  for(j=0 ;j<3; j++)
	      ...
	      ZMW(zmw_box_vertical())
		{
		  ...
		  for(i=0; numbers[j][i]>=0; i++)
			...
			{
			case Zmw_Drag_No: /* Not being dragged */
			  break ;
			case Zmw_Drag_Begin: /* Start being dragged */
			  zmw_drag_from_data_set(buf) ;	/* Set drag data */
			  drag_from_j = j ;
			  drag_from_i = i ;
			  break ;
			case Zmw_Drag_Running: /* Being dragged */
			  ZMW(zmw_window_drag(&w2))
			    {
			      ZMW(zmw_box_vertical())
				{
				  zmw_text(buf) ;
			  ...
			  break ;
			case Zmw_Drag_Accepted: /* The widget was accepted */
			  drag_from_j = -1 ;
			  drag_from_i = -1 ;
			  break ;
			case Zmw_Drag_Refused: /* The widget was refused */
			  drag_from_j = -1 ;
			  drag_from_i = -1 ;
			  break ;
			default:
		...
		{
		case Zmw_Drag_Running: /* The widget if over us */
		  k = j==0
		    || (j==1 && (atoi(zmw_drag_to_data_get())%2) == 0)
		    || (j==2 && (atoi(zmw_drag_to_data_get())%3) == 0)
		    ;
		  drag_to = j ;
		  drag_to_k = k ;		  
		  break ;
		case Zmw_Drag_Ending: /* The widget is released on us */
		  k = j==0
		    || (j==1 && (atoi(zmw_drag_to_data_get())%2) == 0)
		    || (j==2 && (atoi(zmw_drag_to_data_get())%3) == 0)
		    ;
		  zmw_drag_to_accept(k) ; /* Accept/Refuse incoming widget */
		  if ( k )
		    {
		      for(i=0; numbers[j][i]>=0; i++)
			{
			}
		      numbers[j][i++] = atoi(zmw_drag_to_data_get()) ;
		      numbers[j][i++] = -1 ;
		    }
		  drag_to = -1 ;
		  break ;

		case Zmw_Drag_No: /* No widget over us */
		  if ( j == drag_to )
		    {
		      drag_to = -1 ;
		    }
