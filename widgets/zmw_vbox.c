...
/*
 * This file is used to create "zmw_hbox.c"
 ...
 *
 */
void zmw_vbox_required_size(int focusable)
{
  int height, width, i ;
  Zmw_Rectangle r ;
...
void zmw_vbox_children_allocated_size(int focusable)
{
  ...
  for(i=0; i<zmw_nb_of_children_get(); i++)
	  ...
	  if ( first )
	      ...
	      if ( focusable )
		{
		  ...
		  *zmw_child__allocated_x_get_ptr(i) += zmw_focus_width_get() ;
	      ...
	      if ( nb_expandable == 0 )
		{
		  if ( zmw_cs_vertical_alignment_get() >= 0 )
		    {
		      if ( zmw_cs_vertical_alignment_get() > 0 )
			zmw_child__allocated_y_set
			  (i,
			   zmw_allocated_y_get()
			   + zmw_child__padding_width_get(i)
			   + ( zmw_allocated_height_get()
			       - zmw_required_height_get() )
			   ) ;
		      else
			zmw_child__allocated_y_set
			  (i,
	    ...
	    {
	      zmw_child__allocated_y_set(i,
					 zmw_child__allocated_y_get(last)
					 + zmw_child__allocated_height_get(last)
	...
	{
	  zmw_child__allocated_x_set(i, zmw_allocated_x_get()
				     + zmw_child__padding_width_get(i)
				     + zmw_child__required_x_get(i)
				     ) ;
	  zmw_child__allocated_width_set(i, zmw_allocated_x_get()
	...
	{
	  zmw_alignment_horizontal_make(i, focusable ? zmw_focus_width_get() : 0);
	}
	...
	{
	  zmw_child__allocated_height_set
	    (i,
	     zmw_child__required_height_get(i)
	     + (zmw_allocated_height_get()
		- zmw_min_height_get())/nb_expandable /* Can't be a 0 divide */
	     ) ;
	}
      else
	{
...
void zmw_vbox_with_activable(Zmw_Boolean activable)
{
  if ( activable )
    {
      zmw_focusable() ;
  ...
  switch( zmw_subaction_get() )
    ...
    case Zmw_Compute_Required_Size:
      zmw_vbox_required_size(activable) ;
      break ;
    ...
    case Zmw_Compute_Children_Allocated_Size:
      zmw_vbox_children_allocated_size(activable) ;
      break ;
...
void zmw_vbox()
{
  zmw_vbox_with_activable(Zmw_False) ;
}

void zmw_vbox_activable()
{
  zmw_vbox_with_activable(Zmw_True) ;
}
...
void zmw_alignment_vertical_make(int i, int border_width)
{
  ...
  if ( ! zmw_child__verticaly_expanded_get(i) )
    {
      free_space = zmw_allocated_height_get()
	- 2 * border_width
	- zmw_child__required_height_get(i)
	- 2 * zmw_child__padding_width_get(i) ;
      *zmw_child__allocated_height_get_ptr(i) -= free_space ;
      switch(zmw_child__vertical_alignment_get(i))
	    {
	    case -1:
	      break ;
	    case 0:
	      *zmw_child__allocated_y_get_ptr(i) += free_space / 2 ;
	      break ;
	    case 1:
	      *zmw_child__allocated_y_get_ptr(i) += free_space ;
	      break ;
	    }
    }
}
